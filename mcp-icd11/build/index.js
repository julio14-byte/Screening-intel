...
const server = new McpServer({
    ...
    instructions: "Terminología clínica ICD-11 (OMS). Usa icd11_search para buscar códigos y icd11_get_entity para el detalle.",
});
server.registerTool("icd11_search", {
    description: "Busca entidades clínicas en ICD-11 (linearización MMS de la OMS) por texto libre.",
    inputSchema: {
    ...
    try {
        ...
        const text = results
            .map((result) => `• ${result.code ?? "(sin código)"} — ${result.title}${result.matchingText ? ` [coincide: ${result.matchingText}]` : ""}\n  id: ${result.id}`)
            .join("\n");
...
server.registerTool("icd11_get_entity", {
    description: "Obtiene el detalle de una entidad ICD-11 por código o id/URI de icd11_search.",
    inputSchema: {
        idOrCode: z
            .string()
